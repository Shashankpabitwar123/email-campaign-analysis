...
function ResultView({rows,effects,direct,diag}){
 ...
 return <>
   <DataComponent id="result-summary" queryId="campaign_summary" queryIds={['campaign_summary','campaign_effects']} kind="narrative" title="Campaign overview" displayRows={storyRows} sourceRowsByQuery={{campaign_summary:rows,campaign_effects:effects}} variant="plain">
...
function MethodsView({quality,regression,sensitivity,balance,diag}){
 ...
 <Section id="downloads-title" title="Explore the project">
  ...
  <details className="eca-glossary"><summary>Statistical terms</summary><dl><dt>Percentage point</dt><dd>An absolute difference between rates. A rise from 1% to 1.5% is +0.5 percentage points, or +50% relative lift.</dd><dt>Confidence interval</dt><dd>A range produced by a method designed to cover the true effect at its stated rate over repeated comparable samples. Wider ranges mean less precision.</dd><dt>Control group</dt><dd>Customers randomly assigned no email, used to estimate purchases that would happen without a campaign.</dd><dt>Bootstrap</dt><dd>Repeatedly resampling observed customers within each arm to examine variation in the estimated revenue effect.</dd><dt>Multiple comparisons</dt><dd>Testing more than one hypothesis raises the chance of a false positive. The primary analysis corrects for its two planned comparisons.</dd></dl></details>
 </Section>
 </>;
}
